...
The Kubernetes cluster will require provisioning with sufficient computational resources to function. Typically clusters are sized on the basis of total limited capacity (that is, enough to run all components with the headroom to scale).
...
For example, MySQL is provisioned with our Helm-based deployment, but this could equally be configured to reference a provider-managed instance provisioned outside of Helm.
...
"Reserved" instances are a [feature](https://aws.amazon.com/ec2/pricing/reserved-instances/) of AWS that are conceptually common across cloud providers. "Reserved" instances provide the ability to "reserve" capacity in advance of utilization at a significant discount as opposed to paying for on-demand instances.

While Axway does not provide specific guidance for using reserved versus on-demand capacity purchasing we do recommend customers investigate this approach as a means to reduce their cloud infrastructure costs.
...
Our production solution is scaled for 1 million transactions per month (in this context "transactions" means traffic being terminated at the API Gateway and serving open banking requests).
...
The default tolerance is based on utilization. At a given level of utilization new instances will be initialized or destroyed as appropriate.
...
* Use those datasets to guide an "average" view of data transferal (that is the response payloads being transferred from the bank to the cloud provider and then from the cloud provider to the TPP). Note that a different cost profile is likely to be incurred for the first of these if the connection between the bank and the cloud provider is over a VPN (for example, as described by AWS [here](https://aws.amazon.com/vpn/pricing/)).

The following expected behaviors drawn from experiences in the UK should also be noted:
...
* For use cases that drive real-time decisions, such as checking a balance, TPPs are likely to make multiple requests per day but the data volume is likely to be low.
...
In summary, the account information API is likely to result in the largest amount of traffic and if this is being supported should be the focus of customers sizing estimates.

#### Storage

Cloud providers have different storage types that are optimized for various workloads. Customers will have a variety of different workload needs depending on components of Axway Open Banking they adopted.
