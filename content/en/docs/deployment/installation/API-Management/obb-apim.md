...
<!--
The following parameters are required for OBB deployment, so update the `local_values/OBB-YAML/values.yaml` file.
...
Make sure that API Gateway pods i.e. anm, apimgr, apitraffic are in running state and then execute following commands:
...
Log in to API Manager UI to change the default password of `apiadmin` user. It is required for successful execution of below steps.
...
| global.dockerRegistry.url | URL fot the docker registry. | None |
...
4. Check KPS data in API Gateway Manager `https://api-gateway-manager.<domain-name>`:
    * Login and go to Settings - Key Property Stores.
